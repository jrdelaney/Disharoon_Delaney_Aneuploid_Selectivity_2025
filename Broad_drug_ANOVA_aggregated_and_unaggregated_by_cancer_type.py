...
from statsmodels.stats.multitest import multipletests  # ADD
...
def analyze_by_cancer_type(cancer_type, df, chromosome_arms, treatment_col):
    ...
    # Loop through data by arm and check if there's data and run analysis if so
    for arm in chromosome_arms:
        ...
        for treatment in treatments_for_arm:
            ...
            if result:
                print(f"Generated results for Treatment: {treatment}, Chromosome Arm: {chrom_arm_col}, Cancer Type: {cancer_type}")
...
def analyze_aggregate(df, chromosome_arms, treatment_col):
    ...
    for arm in chromosome_arms:
        ...
        for treatment in treatments_for_arm:
            ...
            if result:
                print(f"Generated results for Treatment: {treatment}, Chromosome Arm: {chrom_arm_col}, Cancer Type: ALL")
                results.append(result)
...
def main():
    ...
    # ---------------------------
    # Perform BH multiple-testing correction (cancer-type-specific results)
    # ---------------------------
    ...
    # Save the cancer-type-specific results to CSV
    results_csv_path = os.path.join(SAVE_DIR, 'Final_Broad_Drug_V_Cancer_Type_V_Aneuploidy_Results.csv')
    results_df.to_csv(results_csv_path, index=False)
    print(f"Analysis with aggregation by cancer type completed. Results saved to '{results_csv_path}'.")

    # ---------------------------
    # Now, run the aggregate analysis (no split by cancer type)
    # ---------------------------
    ...
    # Save the aggregate results to a separate CSV
    agg_results_csv_path = os.path.join(SAVE_DIR, 'Final_Broad_Drug_V_ALL_Cancers_Aneuploidy_Results.csv')
